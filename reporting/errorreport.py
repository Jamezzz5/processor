...
class ErrorReport(object):
    ...
    def _dedupe_columns(self, df, label):
        """Drop duplicate column labels so pd.merge can resolve keys.

        Raw files that already contain a 'Full Placement Name' column
        collide with the one generated by full_placement_creation(),
        which otherwise raises ValueError('not unique') on merge.
        """
        if df is None or not df.columns.duplicated().any():
            return df
        dupes = df.columns[df.columns.duplicated()].tolist()
        logging.warning(
            'Duplicate columns {} in {} for {}. Keeping first '
            'occurrence.'.format(dupes, label, self.filename))
        return df.loc[:, ~df.columns.duplicated()]

    def create(self):
        self.df = self._dedupe_columns(self.df, 'data')
        self.dictionary = self._dedupe_columns(
