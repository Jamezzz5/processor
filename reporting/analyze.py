...
class CheckAutoDictOrder(AnalyzeBase):
    ...
    def do_analysis_on_data_source(self, source, df, ven_list=None,
        ...
        ven_auto_dict_idx = (source.p[vmc.autodicord].index(dctc.VEN)
                         if dctc.VEN in source.p[vmc.autodicord] else None)
        camp_auto_dict_idx = (source.p[vmc.autodicord].index(dctc.CAM)
                             if dctc.CAM in source.p[vmc.autodicord] else None)
