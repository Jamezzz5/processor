...
class AliSearch:
    ...
    def __init__(self, ali_chat, transformer=None, transformer_dict=None):
        """
        :param ali_chat: AliChat instance (provides find_db_model,
            remove_stop_words_from_message, get_unigrams_and_bigrams)
        :param transformer: TfIdfTransformer instance (optional, loaded
            from TransformerModel on the app side)
    ...
    def search(self, db_model, message, top_k=5):
        """Combined search returning {model_id: score} dict.
        ...
        :returns: Dict of {model_id: combined_score}
        """
        results = {}
        self._live_search(results, db_model, message)
        if self.transformer is not None and self.transformer_dict:
            self._transformer_search(results, db_model, message, top_k)
    ...
    def _live_search(self, results, db_model, message):
        ...
        try:
            model_ids, words = self.ali_chat.find_db_model(db_model, message)
            if model_ids:
                max_score = max(model_ids.values())
                for model_id, score in model_ids.items():
                    normalized = (score / max_score * WEIGHT_LIVE
                                  if max_score > 0 else 0)
                    results[model_id] = results.get(model_id, 0) + normalized
        except Exception as e:
            logger.warning('Live search failed for {}: {}'.format(
                db_model.__name__, e))

    def _transformer_search(self, results, db_model, message, top_k):
        """Run BM25 + TF-IDF cosine from the provided transformer."""
        try:
            ...
            _merge_transformer_scores(
                results, bm25_scores, self.transformer_dict, model_name,
                weight=WEIGHT_BM25)
            _merge_transformer_scores(
                results, tfidf_scores, self.transformer_dict, model_name,
                weight=WEIGHT_TFIDF)
        except Exception as e:
            logger.warning('Transformer search failed: {}'.format(e))


def _merge_transformer_scores(results, scores, text_dict, model_name,
                              weight=0.5):
    """Merge transformer search scores into the results dict.

    Normalizes scores and filters to the target model type.

    :param results: Dict of {model_id: score} to update in place
